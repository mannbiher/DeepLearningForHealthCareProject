...
def shuffle_five_folds(case_list):
    ...
    train_data = [train_list_1, train_list_2,
                  train_list_3, train_list_4, train_list_5]
    valid_data = [valid_list_1, valid_list_2,
                  valid_list_3, valid_list_4, valid_list_5]
...
def write_five_folds(train_data, valid_data, test_list, out_dir):
    for index, (train_list, valid_list) in enumerate(zip(train_data, valid_data)):
        ...
        for x in test_list:
            test_s[x[2]] += 1
        print(train_s)
        print('N of Train', len(train_list), 'covid:%d' %
              train_s[0], 'pneumonia_virus:%d' % train_s[1], 'pneumonia_bacteria:%d' % train_s[2], 'normal:%d' % train_s[3])
        print('N of Valid', len(valid_list), 'covid:%d' %
              valid_s[0], 'pneumonia_virus:%d' % valid_s[1], 'pneumonia_bacteria:%d' % valid_s[2], 'normal:%d' % valid_s[3])
        print('N of Test', len(test_list), 'covid:%d' %
              test_s[0], 'pneumonia_virus:%d' % test_s[1], 'pneumonia_bacteria:%d' % test_s[2], 'normal:%d' % test_s[3])

        with open(os.path.join(out_dir, f'data_statistic{index+1}.csv'), 'w') as f:
            csv_writer = csv.writer(f)
            csv_writer.writerow(['N of Train', len(train_list), 'covid:%d' % train_s[0], 'pneumonia_virus:%d' %
                                train_s[1], 'pneumonia_bacteria:%d' % train_s[2], 'normal:%d' % train_s[3]])
            csv_writer.writerow(['N of Valid', len(valid_list), 'covid:%d' % valid_s[0], 'pneumonia_virus:%d' %
                                valid_s[1], 'pneumonia_bacteria:%d' % valid_s[2], 'normal:%d' % valid_s[3]])
            csv_writer.writerow(['N of Test', len(test_list), 'covid:%d' % test_s[0], 'pneumonia_virus:%d' %
                                test_s[1], 'pneumonia_bacteria:%d' % test_s[2], 'normal:%d' % test_s[3]])

            train_path = os.path.join(
                out_dir, 'exp_train_list_cv%d.pkl' % (index+1))
            valid_path = os.path.join(
                out_dir, 'exp_valid_list_cv%d.pkl' % (index+1))
            test_path = os.path.join(
                out_dir, 'exp_test_list_cv%d.pkl' % (index+1))
...
def setup_cli():
    ...
    parser.add_argument('--kaggle', type=str,
                        help='Kaggle pickle file', required=True)
    parser.add_argument('--covid', type=str,
                        help='Covid pickle file', required=True)
    # Datasets
