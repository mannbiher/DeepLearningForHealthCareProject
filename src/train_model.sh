...
for i in $(seq 1 5); do
    ...
    python FLANNEL/ensemble_step2_ensemble_learning.py --epochs=$i -ck_n=$ck_n --data_dir='./explore_version_03/results/%s_20200407_multiclass_%s' --cv=cv$i -j=$workers
    python FLANNEL/ensemble_step2_ensemble_learning.py --epochs=$i -ck_n=$ck_n --data_dir='./explore_version_03/results/%s_20200407_multiclass_%s' --cv=cv$i -j=$workers --test
done
