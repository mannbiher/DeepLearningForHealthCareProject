...
ck_n=50
for i in $(seq 1 5); do
    python FLANNEL/ensemble_step1.py --arch inception_v3 --epochs=$epochs --crop_size=299 -ck_n=$ck_n --cv=cv$i
    python FLANNEL/ensemble_step1.py --arch inception_v3 --epochs=$epochs --crop_size=299 -ck_n=$ck_n --cv=cv$i --test
    python FLANNEL/ensemble_step1.py --arch resnext101_32x8d --epochs=$epochs -ck_n=$ck_n --cv=cv$i
    python FLANNEL/ensemble_step1.py --arch resnext101_32x8d --epochs=$epochs -ck_n=$ck_n --cv=cv$i --test
#    python FLANNEL/ensemble_step1.py --arch resnet152 --epochs=50 -ck_n=10 --cv=cv$i
